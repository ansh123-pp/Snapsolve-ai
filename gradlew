...
if $cygwin ; then
    ...
    if [ "$GRADLE_OPTS" != "" ] ; then
        GRADLE_OPTS="$GRADLE_OPTS \"-Xdock:name=$APP_NAME\" \"-Xdock:name=$APP_NAME\""
    fi
...
# might be in the subdir of the workspace. Let's try to locate the gradle wrapper in such cases (we should be in the root of the project).
